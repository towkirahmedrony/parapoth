    ...
    <Card className="rounded-3xl border border-card-border bg-card-bg p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-2xl font-bold text-text-primary">{props.value}</h3>
        </div>
        <div className="rounded-2xl bg-secondary p-3 text-text-primary">
          {props.icon}
        </div>
      </div>
    </Card>
...
function InfoBadge(props: { children: React.ReactNode }) {
  return (
    <span className="rounded-full border border-border-color bg-badge-bg px-2.5 py-1 text-xs font-medium text-badge-text">
      {props.children}
    </span>
  );
}
...
function QuestionCard(props: {
  ...
  return (
    <Card className="rounded-3xl border border-card-border bg-card-bg p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex flex-col gap-4">
        ...
        <div>
          <h3 className="line-clamp-3 text-base font-semibold leading-7 text-text-primary">
            {preview}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <div className="rounded-2xl bg-surface p-3">
            ...
            <p className="mt-1 text-sm font-semibold text-text-primary">{question.totalAttempts}</p>
          </div>
          <div className="rounded-2xl bg-surface p-3">
            <p className="text-xs text-text-secondary">Correct</p>
            <p className="mt-1 text-sm font-semibold text-text-primary">{question.correctAttempts}</p>
          </div>
          <div className="rounded-2xl bg-surface p-3">
            <p className="text-xs text-text-secondary">Accuracy</p>
            <p className="mt-1 text-sm font-semibold text-text-primary">{accuracy}%</p>
          </div>
          <div className="rounded-2xl bg-surface p-3">
            <p className="text-xs text-text-secondary">Question ID</p>
            <p className="mt-1 truncate text-sm font-semibold text-text-primary">{question.id}</p>
          </div>
        </div>
  ...
  return (
    <div className="min-h-full bg-app px-4 py-4 text-text-primary sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="overflow-hidden rounded-[28px] border border-card-border bg-card-bg shadow-sm">
          <div className="flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl">
              <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-secondary px-3 py-1 text-xs font-medium text-text-primary">
        ...
        <section className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <main className="space-y-4">
            ...
            {isLoading ? (
              <div className="flex min-h-[320px] items-center justify-center rounded-3xl border border-card-border bg-card-bg">
                <Loader />
              </div>
            ) : error ? (
              <Card className="rounded-3xl border border-border-color bg-card-bg p-8">
              ...
              <div className="flex flex-col items-center justify-center py-10">
                <Search className="h-10 w-10 text-text-secondary mb-4" />
                <EmptyState
