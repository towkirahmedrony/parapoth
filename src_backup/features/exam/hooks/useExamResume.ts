...
export const useExamResume = () => {
  ...
  const saveProgress = useCallback((examId: string, state: ExamProgressState, userId?: string) => {
    ...
    // 2. Supabase sync via mutation
    syncProgressMutation.mutate({ examId, state, userId });
  ...
  const loadProgress = useCallback(async (examId: string, userId?: string): Promise<ExamProgressState | null> => {
    ...
    try {
      return StorageUtils.get<ExamProgressState>(key);
    } catch (error) {
  ...
  const clearProgress = useCallback((examId: string, userId?: string) => {
    try {
      const key = `${STORAGE_KEYS.EXAM_PROGRESS_PREFIX}${examId}` as AnyStorageKey;
    ...
    clearProgressMutation.mutate({ examId, userId });
