...
const fetchArenaQuestions = async (limit: number): Promise<ExamQuestion[]> => {
  const res = await apiClient.get(`/exams/user/arena?limit=${limit}`);
...
const Arena: React.FC = () => {
  ...
  const { data: questions = [], isLoading } = useQuery({
    queryKey: ['arenaQuestions', config.questionCount],
    queryFn: () => fetchArenaQuestions(config.questionCount),
    staleTime: Infinity, 
  ...
  const reportMutation = useMutation({
    mutationFn: async (reportData: ModalReportPayload) => {
      await apiClient.post(`/system/report`, reportData);
    },
    onSuccess: () => {
  ...
  const processSubmission = useCallback(() => {
    ...
    const timeTaken = (config.duration * 60) - Math.max(0, timeLeft);
  ...
  }, [questions, userAnswers, config, timeLeft, submitExam]);
  ...
  // Promise রিটার্ন করা হচ্ছে
  const handleReportSubmit = useCallback(async (reportData: ModalReportPayload) => {
    await reportMutation.mutateAsync(reportData);
    // ReportModal এখন নিজেই onClose কল করবে সাবমিট হওয়ার পর, তাই এখানে handleCloseReportModal ডাকার প্রয়োজন নেই।
  }, [reportMutation]);
