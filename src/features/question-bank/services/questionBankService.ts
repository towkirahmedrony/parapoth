...
function applyClientSideFilters(
  ...
  if (filters.mistakesOnly) {
    result = result.filter((item) => item.isWrongAnswered);
  }
...
export async function getQuestionBankFilters(): Promise<QuestionBankFiltersData> {
  const [{ data: subjects, error: subjectsError }, { data: chapters, error: chaptersError }, { data: topics, error: topicsError }] =
    await Promise.all([
      supabase
        .from('subjects')
        .select('id, name_bn, name_en, slug, is_premium, sequence')
        .eq('is_active', true)
        .order('sequence', { ascending: true }),
      supabase
        .from('chapters')
        .select('id, subject_id, name_bn, name_en, slug, is_premium, sequence')
        .eq('is_active', true)
        .order('sequence', { ascending: true }),
      supabase
        .from('topics')
        .select('id, chapter_id, name_bn, name_en, slug, is_premium, sequence, total_questions')
        .eq('is_active', true)
        .order('sequence', { ascending: true }),
    ]);
...
export async function getQuestionBankQuestions(
  ...
  if (filters.questionType) {
    query = query.eq('type', filters.questionType);
  }
