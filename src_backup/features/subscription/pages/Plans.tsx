...
const Plans: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20" style={{ backgroundColor: 'var(--dyn-bg)', color: 'var(--dyn-text)' }}>
      {/* Hero Section */}
      <div 
        className="pt-12 pb-24 px-4 rounded-b-[3rem] text-white relative overflow-hidden"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {isLoading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div 
                key={`skeleton-${i}`} 
                className="h-96 rounded-2xl p-6"
